...
class CompositeGraphTensor:
    ...
    def fromGraphObject(self, g: CompositeGraphObject):
        ...
        return self(nodes=g.nodes, arcs=g.arcs, targets=g.targets, type_mask=g.type_mask.transpose(),
                    ...
                    sample_weights=g.sample_weights, NodeGraph=self.COO2SparseTensor(g.NodeGraph),
                    Adjacency=self.COO2SparseTensor(g.Adjacency),
    ...
    def COO2SparseTensor(coo_matrix) -> tf.Tensor:
        ...
        indices = list(zip(coo_matrix.row, coo_matrix.col))
        if not indices: indices = np.zeros(shape=(0, 2), dtype=int)
