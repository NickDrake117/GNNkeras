...
#######################################################################################################################
def print_setup(dir):
    setup = {i: eval(i) for i in dir if '__' not in i and 'module' not in str(eval(i))}
...
graphs = CompositeGraphObject.load_dataset("Bongio", focus='g')
...
gVa = gVa.tolist()
gTe = gTe.tolist()
gGen = CompositeGraphObject.merge(gTr, aggregation_mode=graph_aggregation_mode, focus=graph_focus)

'''if graph_scalers:
    scalers = gGen.normalize(graph_scalers, True, True)
    for g in gTr+gVa+gTe: g.normalize_from(scalers)'''
...
nets_state = [[MLP(input_dim=s, layers=j, activations=net_state_activations,
                   ...
                   name=f'State_{idx}')
               for s in i]
              for idx, (i, j) in enumerate(zip(net_state_input, net_state_layers))]
...
nets_output = [MLP(input_dim=k, layers=j, activations=net_output_activations,
                   ...
                   name=f'Out_{idx}')
               for idx, (i, j) in enumerate(zip(net_output_input, net_output_layers))
               for k in i]
...
# lgnn.fit(training_sequencer, epochs=epochs, validation_data=validation_sequencer, callbacks=callbacks_lgnn)
